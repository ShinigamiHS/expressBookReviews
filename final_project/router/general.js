const express = require('express');
let books = require("./booksdb.js");
let isValid = require("./auth_users.js").isValid;
let users = require("./auth_users.js").users;
const public_users = express.Router();


public_users.post("/register", (req, res) => {
  const username = req.body.username;
  const password = req.body.password;
  if (!username) {    // Return error if username or password is missing
    return res.status(404).json({ message: "Username missing" });
  }
  if (!password) {
    return res.status(404).json({ message: "Password missing" });
  }
  if (!isValid(username)) {    // Check if both username and password are provided
    // Add the new user to the users array
    users.push({ "username": username, "password": password });
    return res.status(200).json({ message: "User successfully registered. Now you can login" });
  } else {
    return res.status(404).json({ message: "User already exists!" });
  }
});


// Get the book list available in the shop
public_users.get('/', function (req, res) {
  const bookPromise = new Promise((resolve, reject) => {
    resolve(res.send(JSON.stringify(books, null, 2)));
  })
  bookPromise.then(() => {
    console.log("Book List retrieved");
  })
});

// Get book details based on ISBN
public_users.get('/isbn/:isbn', function (req, res) {
  const isbn = req.params.isbn;

  const isbnPromise = new Promise((resolve, reject) => {
    resolve(res.send(books[isbn]));
  })
  isbnPromise.then(() => {
    console.log(`Book ${isbn} retrieved`);
  })
});

// Get book details based on author
public_users.get('/author/:author', function (req, res) {
  const author = req.params.author;
  let answerArray = {};

  console.log(author)

  for (const key in books) {
    if (books[key].author === author) {
      answerArray[key] = books[key];
    }
  }
  console.log(JSON.stringify(answerArray).length);

  const authorPromise = new Promise((resolve, reject) => {
    if (JSON.stringify(answerArray).length === 2) {
      console.log("nope");
      resolve(res.send(`Author ${author} not found`));
    }
    else {
      resolve(res.send(JSON.stringify(answerArray, null, 2)));
    }
  });
});

// Get all books based on title
public_users.get('/title/:title', function (req, res) {
  const title = req.params.title;
  const answerArray = {};

  console.log(title)
  console.log(books.length);

  for (const key in books) {
    if (books[key].title === title) {
      answerArray[key] = books[key];
    }
  }
  const titlePromise = new Promise((resolve, reject) => {

    if (JSON.stringify(answerArray).length === 2) {
      resolve(res.status(400).json({ message: "title not found" }));
    } else {
      resolve(res.send(JSON.stringify(answerArray, null, 2)));
    }
  });
});

//  Get book review
public_users.get('/review/:isbn', function (req, res) {
  const isbn = req.params.isbn;

  res.send(books[isbn].reviews);

  return res.status(300).json({ message: "Yet to be implemented" });
});

module.exports.general = public_users;
